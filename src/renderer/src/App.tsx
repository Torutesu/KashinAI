...
function AssistantFlow() {
  ...
  async function autoRecommendForContext(nextContext: CurrentContext, autoInsert: boolean): Promise<void> {
    const recommendationRequest: ChatMessage = {
      role: 'user',
      content:
        '現在の画面コンテキスト、スクリーンOCR、選択テキストを最優先で読んで、今すぐ入力欄に貼り付けて使えるおすすめ文を1つ作ってください。Twitter/Xなら表示中の投稿や返信欄に合う短い投稿・返信文、コード画面ならそのコードやエラーに合う短いコメント・説明・次アクションにしてください。会社メモやGBrainは、現在画面と明確に関係がある場合だけ参考にしてください。前置き、見出し、引用符、説明、Context used、箇条書きラベルは出さず、貼り付ける本文だけを出力してください。'
    }
