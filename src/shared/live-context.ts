...
function socialScore(line: string): number {
  if (NAV_LINE_RE.test(line) || SOCIAL_NOISE_RE.test(line)) return -20
  let score = 0
...
function bestLines(lines: string[], kind: CurrentContext['contextKind'], maxLines: number): string[] {
  ...
  if (kind === 'coding') {
    const scored = lines
      ...
      .sort((a, b) => a.index - b.index)
      .map((item) => item.line)
    return scored.length > 0 ? scored : lines.slice(0, maxLines)
  }

  return lines.filter((line) => !NAV_LINE_RE.test(line)).slice(0, maxLines)
}
